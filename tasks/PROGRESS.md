...
- **已确认事实**：
  ...
  2. 客户端控制台一致报 `Failed to fetch manifest patches Error: 500`（source: /assets/chunk-62JRHF6Z-*.js）——react-router v7.18 路由发现（routeDiscovery lazy + manifestPath=/__manifest）请求失败
  3. 静态路由 HTML 预渲染完整：/download /about /docs /commands 均 200 且内容完整（/docs 16 篇全列、/commands 指令卡齐全）
  4. worker.ts 对非 /api/chat 请求仅 `env.ASSETS.fetch(request)`（111 行）——**铁律不可改 worker.ts/ai-stream.ts**
...
- **（追加 2）desktop 恢复应用内下载 + 去按钮「推荐」tag**（2026-08-06，1e518896）：desktop 两按钮改回 `downloadFromUrl` 应用内下载（staging 目录，安装页「已下载配置包」直接用，带下载 spinner）；按钮「推荐」字样+遮罩删除（website 同步删按钮推荐 tag，信息箱「镜像，推荐」→「镜像」），仅颜色区分来源。v3.1.10 tag 第 3 次覆盖重发（1e518896），3 工作流全绿，Release 资产刷新（15:10）
