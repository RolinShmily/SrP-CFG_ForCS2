...
export default function DownloadsList({ selectedFolder, onSelect }: Props) {
  ...
  const loadEntries = useCallback(async () => {
    setLoading(true);
    try {
      const list = await window.api.getDownloadEntries();
      setEntries(list);
    } finally {
      setLoading(false);
    }
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 text-text-faint">
        ...
        <p className="text-xs">暂无已下载的配置包</p>
        <p className="text-xs mt-0.5">在下载页面获取配置包</p>
      </div>
    );
  }
