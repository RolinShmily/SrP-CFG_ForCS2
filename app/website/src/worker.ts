...
//
// Qwen3 是推理模型，思考内容会计入 max_tokens 并可能挤掉正文，因此下面的 system prompt
// 末尾带上 /no_think 软开关；.github/scripts/test_worker_llm.py 会在预检里实测是否真的
// 没有思考内容泄漏。
...
function formatCommandContext(item: CommandMetadata): string {
  return [
...
async function handleChat(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    // 2. Query the official command index.
    const indexToQuery = env.COMMANDS_INDEX;
    if (!indexToQuery) {
      return jsonError("指令库暂时不可用。", 503);
    }
    ...
    // 3. Build the reference context from retrieved command records.
    const referenceContext = (vectorMatches.matches ?? [])
      .map((match) => match.metadata)
      .filter(isCommandMetadata)
      .map(formatCommandContext)
      .join("\n") || "未检索到匹配记录。";

    // 4. Construct the prompt.
    const systemPrompt = `你是 CS2 官方控制台指令与变量助手。下方参考资料来自独立的官方指令向量库。
    ...
    try {
      stream = (await env.AI.run(
        ...
        },
        {
          gateway: {
            id: "srp-cfg",
            metadata: { knowledgeBase: "commands" },
          },
        },
      )) as ReadableStream;
