...
2. 在安装可能应用偏好或键位的未知自定义 CFG 前保存账号级 JSON 基线。
3. 阻止 `.vcfg`、`.vcfg_lastclouded` 与 `remotecache.vdf` 进入暂存和发行包。
4. 明确区分 Runtime 回滚、安装前原文件、User 保护与 VCFG 状态快照。
...
未来若实现恢复，安全方向应是“由快照生成恢复 CFG，让游戏自己执行 bind/ConVar 命令”，而不是替游戏覆盖序列化文件。
