...
crosshair_view.cfg 默认随 autoexec.cfg 启动，提供准星预设切换、持枪视角切换、准星颜色切换、投掷物准星开关等功能。

由于 Valve 的脚本指令单条长度限制，准星参数需通过 cfg 文件加载。更换准星必须搭配 `crosshair_library` 准星代码库使用。
...
1. 默认随 autoexec 启动，无需额外操作
2. 按 `7` 键轮换准星预设，按 `8` 键轮换视角预设
3. 控制台输入 `c00`~`c07` 切换指定准星，`v00`~`v07` 切换指定视角
4. 控制台输入颜色别名（red、blue 等）切换准星颜色
5. 输入 `whamic` 或 `whamiv` 查询当前预设

## 相关文件

- [autoexec.cfg](/docs/autoexec) — 主配置文件，启动时加载本文件
- `crosshair_library/` — 准星预设 cfg 文件夹（00.cfg ~ 07.cfg）
