...
guidemake.cfg 用于在 CS2 练习模式下创建投掷物标识地图指南。创建的标识可在游戏内查看，也可上传至创意工坊分享给其他玩家。

## 激活方式

控制台输入 `exec guidemake` 加载。需要在练习模式下使用。
...
| 上传至创意工坊 | `upload` | 控制台输入，原始指令: `workshop_annotation_submit` |
...
- [practice.cfg](/docs/practice) — 跑图配置（配合使用）
