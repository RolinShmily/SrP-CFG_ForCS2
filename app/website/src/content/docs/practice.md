...
| 功能 | 快捷键 | 控制台别名 |
| :--- | :--- | :--- |
| 切换队友头顶显示 | `ralt` | `srp_prac_teamid` |
| 弹着点与道具轨迹预测 | `↑` | `switch-impactsNpreview` |
| 杀死 Bot | `↓` | `srp_prac_bot_kill` |
| 重新开始 | `→` | `srp_prac_restart` |
| 补全护甲 | `F1` | `srp_prac_armor` |
| 自动回血 | `F2` | `srp_prac_regen` |
| 切换人称视角 | `F3` | `switchperson` |
| 添加 Bot | `F4` | `srp_prac_bot_add` |
| 使 Bot 蹲下 | `F5` | `srp_prac_bot_crouch` |
| 使 Bot 模仿操作 | `F6` | `srp_prac_bot_mimic` |
| 视角放大镜 | `F7` | `srp_prac_fov` |
| 删除 Bot | `F8` | `srp_prac_bot_kick` |
| 友伤状态 | `F9` | `srp_prac_teammates_enemies` |
| Bhop 连跳 | `F12` | `srp_prac_bhop` |
| 重现最近一次道具 | `L` | `srp_prac_rethrow` |
| 加速时间流逝 | `0` | `+Pucci` |
| 放置 Bot | `MOUSE5` | `srp_prac_bot_place` |
| 飞行模式 | `N` | `srp_prac_noclip` |
| 显示速度信息 | `K` | `srp_prac_showpos` |
| 实体显示 | `.` | `srp_prac_aoproxy` |
| 弹夹容量状态（无限弹药切换） | `/` | `srp_prac_infammo` |
| 清除场上投掷物 | `,` | `srp_prac_clear_grenades` |
| 清除地图指南标识 | `rshift` | - |
| 标准实战模拟 | - | `gkd` |
| 恢复跑图模式 | - | `gg` |
| 加载出生点预设 | - | `spawn` |
...
- [pwa-prac 完美跑图模式](/docs/pwa_prac) — 专为完美平台跑图服设计的按键映射
