/**
 * Steps —— 四步工作流（对应原 Steps.astro，React 化）。
 * 区块标题用共享 SectionHeader（@srp-cfg/ui）。
 */
import { Download, Gamepad2, PackageCheck, UserRoundCog } from "lucide-react";
  ...
  return (
    <section className="home-section border-b border-border py-20 sm:py-28 lg:py-32">
      <div className="mx-auto max-w-[1280px] px-5 sm:px-7">
        ...
        <ol className="grid grid-cols-1 border-l border-t border-border md:grid-cols-2 xl:grid-cols-4">
          {steps.map((step) => (
            <li
              key={step.num}
            ...
            >
              <div className="flex items-center justify-between">
                <span className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-sm)] border border-border bg-bg-raised text-text-muted">
                  <step.icon className="h-5 w-5" strokeWidth={1.8} />
                </span>
                <span className="font-mono text-xs text-text-faint">{step.num} / 04</span>
              </div>
              <h3 className="mt-8 font-display text-2xl font-bold text-text">{step.title}</h3>
              <code className="mt-3 block break-all font-mono text-xs text-accent-light">
                {step.command}
              </code>
